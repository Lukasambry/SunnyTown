...
const isSelected = computed(() => {
    try {
        ...
        console.error('Error checking building selection:', error)
        return false
    }
})
...
const getCostItemClasses = (resource: string, amount: number) => {
    try {
        ...
        if (!resourceManager) {
            return {
                'bg-gray-600/20 border border-gray-500/30 text-gray-300': true
            }
        }
        ...
        console.error(`Error calculating cost display for ${resource}:`, error)
        return {
            'bg-gray-600/20 border border-gray-500/30 text-gray-300': true
        }
    }
